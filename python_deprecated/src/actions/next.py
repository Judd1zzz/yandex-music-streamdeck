...
@action_handler("com.judd1.yandex_music.action.next")
class NextTrack(YandexMusicBaseAction):
    ...
    async def render_action(self):
        ...
        style = self.settings.get("next_style", "v1")
        
        if mode == "local":
            if not self.cdp.is_connected:
                await self.set_image(f"btn_yandex_music_next_{style}_loading.png")
                return
        ...
        await self.set_image(f"btn_yandex_music_next_{style}.png")

    async def on_key_down(self, obj: KeyDownModel):
        mode = self.settings.get("control_mode", "local")
        if mode == "local":
            await self.cdp.next_track()
        else:
            await self.client.send_command(YnisonCommand.NEXT)
