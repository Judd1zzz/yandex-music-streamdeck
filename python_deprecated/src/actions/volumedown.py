...
@action_handler("com.judd1.yandex_music.action.volumedown")
class VolumeDown(YandexMusicBaseAction):
    """Кнопка уменьшения громкости на 5%"""
    ...
    async def render_action(self):
        ...
        await self.set_image(f"btn_yandex_music_vol_down_{style}.png")

        if mode == "local":
            if not self.cdp.is_connected:
                await self.set_image(f"btn_yandex_music_vol_down_{style}_loading.png")
                return

    async def _change_volume(self):
        mode = self.settings.get("control_mode", "local")
        if mode == "local":
            await self.cdp.change_volume("DOWN")
        else:
            await self.client.send_command(YnisonCommand.VOLUME_DOWN)
